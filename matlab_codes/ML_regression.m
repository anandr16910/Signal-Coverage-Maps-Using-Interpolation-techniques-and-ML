...
parfor i = 1:1:2
    ...
    %mdl_rf = TreeBagger(75, X(trainIdx,:), Y(trainIdx), 'Method', 'regression');
     treeTemplate = templateTree('MinLeafSize', 1, 'MaxNumSplits', []);
    model_rf = fitrensemble(X(trainIdx,:), Y(trainIdx,:), 'Method', 'LSBoost', ...
                                    'NumLearningCycles', 75, ...
                                    'Learners', treeTemplate,'LearnRate',0.14,'Resample','on');
     % Predict RSRP
    Y_pred_rf = predict(model_rf, X(testIdx,:));
