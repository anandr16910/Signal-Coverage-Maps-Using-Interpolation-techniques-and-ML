...
net = feedforwardnet(50,'trainlm'); % 10 hidden neurons
% layers = [
%     featureInputLayer(3)   % Define input layer
%     fullyConnectedLayer(50)        % Hidden layer with 50 neurons
% 
%     reluLayer
%     fullyConnectedLayer(25)  
%     reluLayer
%     %batchNormalizationLayer   % added just now
%     %reluLayer                      % Activation function
%     fullyConnectedLayer(1) % Output layer
%     regressionLayer                 % Regression layer for RSRP prediction
% ];
...
%     'Shuffle', 'every-epoch', ...
%     'Verbose', true, ...
%     'Plots', 'training-progress'); % Shows training progress graphically
...
net.trainParam.epochs = 75;        % Maximum epochs
